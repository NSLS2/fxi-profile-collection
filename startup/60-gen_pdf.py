...
def reset_pdf():
    ...
    PDF_ARGS["C_canvas"] = canvas.Canvas(
        PDF_ARGS["fn_tmp"], pagesize=letter, bottomup=1
    )
...
def check_page_is_full():
    if (
        PDF_ARGS["Cursor_y"] <= inch
    ):  # current canvas is full, need to save it to pdf and create new canvas
        PDF_ARGS["C_canvas"].showPage()
        PDF_ARGS["C_canvas"].save()
        merge_pdf(PDF_ARGS["fn_log"], PDF_ARGS["fn_tmp"], PDF_ARGS["fn_log"])
        PDF_ARGS["f"] = open(
            PDF_ARGS["fn_tmp_txt"], "w"
        )  # write current text to current_log.txt
        reset_pdf()
        #        PDF_ARGS['Cursor_y'] = PDF_ARGS['Height'] - inch
        return 1
    else:
...
def demo_pdf():
    ...
    insert_text(
        "fasdfadsfijhfp;sadiofhwaeiuhfg;ailksdmfcoiawehjfpioqawnfvcaikewmFASDFADSFADSFADSFADSF"
    )
